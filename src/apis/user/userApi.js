...
export async function getUserIngredients() {
  const response = await fetch(`${BASE_URL}/api/users/me/ingredients`, {
    credentials: "include",
  });
...
export async function deleteUserIngredients(ingredientId) {
  const response = await fetch(
    `${BASE_URL}/api/users/me/ingredients/${ingredientId}`,
    {
      credentials: "include",
      method: "DELETE",
    }
...
export async function getUserScrappedRecipes() {
  const response = await fetch(`${BASE_URL}/api/users/me/scraps`, {
    credentials: "include",
  });
...
export async function getUserRecipes() {
  const response = await fetch(`${BASE_URL}/api/users/me/recipes`, {
    credentials: "include",
  });
